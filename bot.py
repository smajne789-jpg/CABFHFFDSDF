...
class CryptoPay:
    ...
    async def get_invoices(self, invoice_ids: list[int]) -> list[dict]:
        if not invoice_ids:
            return []
        result = await self._request("getInvoices", {"invoice_ids": ",".join(map(str, invoice_ids))})
...
async def settle_invoice(invoice: dict) -> None:
    ...
    stored = storage.mark_invoice_paid(invoice_id)
    if not stored:
        return
    ...
    while True:
        try:
            ...
            if active:
                ...
                for row in active:
                    invoice = invoice_map.get(str(row["invoice_id"]))
                    if not invoice:
                        continue
